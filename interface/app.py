...
def get_business():
    ...
    nodes = list(
        map(
            buildNodes,
            graph.run(
                "match (a{name:'" + name + "'})-[b:发行债券|发行股票|总授信额度]-(c) return  "
...
def get_connect():
    ...
    nodes = list(
        map(
            buildNodes,
            graph.run(
                "match (a{name:'" + name1 + "'})-[b:发行债券|发行股票|总授信额度]-(c)-"
...
def get_credit():
    ...
    nodes = list(
        map(
            buildNodes,
            graph.run(
                'match (a{name:"' + name + '"})-[b]-(c:总授信额度)-[d]-(e) '
                "return distinct  id(c) as id, c.name as name, labels(c) as label"
            ),
        )
    )
...
def get_child():
    name = str(request.args.get("name"))
    nodes = graph.run(
        'match (a{name:"' + name + '"})-[:投资占比]->(c) return distinct c.name as name'
    ).data()  # 查询数据信息
