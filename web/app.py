...
def get_business():
    ...
    nodes = list(
        map(
            buildNodes,
            graph.run(
                "match (a:公司名称{name:'" + name + "'})-[b:发行债券|发行股票|总授信额度]-(c) return  "
    ...
    if len(nodes) == 0:
        ...
        current_node = list(
            map(
                buildNodes,
                graph.run(
                    'match (a:公司名称{name:"' + name + '"}) '
                    "return distinct id(a) as id,labels(a) as label ,a.name as name"
                ).data(),
            )
        )
        ...
        edges = list(
            map(
                ...
                graph.run(
                    'match (a:公司名称{name:"'
                    + name
                    + '"})-[b:发行债券|发行股票|总授信额度]-(c) return distinct id(startnode(b)) as start,id(endnode(b)) as end,type(b) as type, b.relationship as relationship, id(b) as id'
                ),
            )
        )
        # print(nodes, edges)
...
def get_connect():
    ...
    nodes = list(
        map(
            buildNodes,
            graph.run(
                "match (a:公司名称{name:'" + name1 + "'})-[b:发行债券|发行股票|总授信额度]-(c)-"
    ...
    nodes1 = list(
        map(
            ...
            graph.run(
                ...
                "(i:公司名称{name:'"
                + name2
                + "'}) return distinct  id(e) as id, e.name as name, labels(e) as label"
            ),
        )
    )
    nodes2 = list(
        map(
            ...
            graph.run(
                ...
                "(i{name:'"
                + name2
                + "'}) return distinct  id(g) as id, g.name as name, labels(g) as label"
            ),
        )
    )
    nodes = nodes + nodes1 + nodes2
        ...
        edges = list(
            map(
                ...
                graph.run(
                    ...
                    "(i:公司名称{name:'"
                    + name2
                    + "'}) return distinct  id(startnode(b)) as start, id(endnode(b)) as end, type(b) as type, b.relationship as relationship,id(b) as id"
                ),
            )
        )
        edges1 = list(
            map(
                ...
                graph.run(
                    ...
                    "(i:公司名称{name:'"
                    + name2
                    + "'}) return distinct  id(startnode(d)) as start, id(endnode(d)) as end, type(d) as type, d.relationship as relationship, id(d) as id"
                ),
            )
        )
        edges2 = list(
            map(
                ...
                graph.run(
                    ...
                    "(i:公司名称{name:'"
                    + name2
                    + "'}) return distinct  id(startnode(f)) as start, id(endnode(f)) as end, type(f) as type, f.relationship as relationship, id(f) as id"
                ),
            )
        )
        edges3 = list(
            map(
                ...
                graph.run(
                    ...
                    "(i:公司名称{name:'"
                    + name2
                    + "'}) return distinct  id(startnode(h)) as start, id(endnode(h)) as end, type(h) as type, h.relationship as relationship, id(h) as id"
                ),
            )
        )
        edges = edges + edges1 + edges2 + edges3
    ...
    nodes = list(
        map(
            ...
            graph.run(
                "match (a:公司名称{name:'"
                + name
                + "'})-[b:高管关系|投资占比]-(c) return distinct id(c) as id, c.name as name, labels(c) as label"
            ),
        )
    )
    nodes = nodes
        ...
        edges = list(
            map(
                ...
                graph.run(
                    "match (a:公司名称{name:'"
                    + name
                    + "'})-[b:高管关系|投资占比]-(c)  return distinct  id(startnode(b)) as start, id(endnode(b)) as end,type(b) as type, b.relationship as relationship, id(b) as id"
                ),
            )
        )
    ...
    nodes1 = list(
        map(
            ...
            graph.run(
                'match (a:公司名称{name:"'
                + name
                + '"})-[b:发行债券|发行股票|总授信额度]-(e)-[d:合作银行|上市推荐人|股票主承销商|债券主承销商|保荐机构]-(c) return distinct id(c) as id, c.name as name, labels(c) as label'
            ),
        )
    )
    nodes = nodes + nodes1
        ...
        edges = list(
            map(
                ...
                graph.run(
                    'match (a:公司名称{name:"'
                    + name
                    + '"})-[b:发行债券|发行股票|总授信额度]-(c)-[d:合作银行|上市推荐人|股票主承销商|债券主承销商|保荐机构]-(e) return distinct   id(startnode(b)) as start, id(endnode(b)) as end,type(b) as type, b.relationship as relationship, id(b) as id'
                ),
            )
        )
        edges1 = list(
            map(
                ...
                graph.run(
                    'match (a:公司名称{name:"'
                    + name
                    + '"})-[b:发行债券|发行股票|总授信额度]-(c)-[d:合作银行|上市推荐人|股票主承销商|债券主承销商|保荐机构]-(e) return distinct  id(startnode(d)) as start, id(endnode(d)) as end,type(d) as type, d.relationship as relationship, id(d) as id'
                ),
            )
        )
        edges = edges + edges1
...
def get_credit():
    ...
    nodes = list(
        map(
            buildNodes,
            graph.run(
                'match (a:公司名称{name:"' + name + '"})-[b]-(c:总授信额度)-[d]-(e) '
                "return distinct  id(c) as id, c.name as name, labels(c) as label"
            ),
        )
    )
    nodes1 = list(
        map(
            ...
            graph.run(
                'match (a:公司名称{name:"'
                + name
                + '"})-[b]-(e:总授信额度)-[d]-(c) return distinct  id(c) as id, c.name as name, labels(c) as label'
            ),
        )
    )
    nodes = nodes + nodes1
        ...
        edges = list(
            map(
                ...
                graph.run(
                    'match (a:公司名称{name:"'
                    + name
                    + '"})-[b]-(c:总授信额度)-[d]-(e) return distinct  id(startnode(b)) as start, id(endnode(b)) as end,type(b) as type, b.relationship as relationship,id(b) as id'
                ),
            )
        )
        edges1 = list(
            map(
                ...
                graph.run(
                    'match (a:公司名称{name:"'
                    + name
                    + '"})-[b]-(c:总授信额度)-[d]-(e) return distinct  id(startnode(d)) as start, id(endnode(d)) as end,type(d) as type, d.relationship as relationship, id(d) as id'
                ),
            )
        )
        edges = edges + edges1
...
def get_child():
    ...
    nodes = graph.run(
        'match (a:公司名称{name:"'
        + name
        + '"})-[:投资占比]->(c) return distinct c.name as name'
    ).data()  # 查询数据信息
    ...
    if len(rel) == 0:  # 关系不存在
        if percent:  # 如果有投资占比
            graph.run(
                ...
                + '"}),(m:公司名称{name:"'
                + child
                + "\"}) create (n)-[r:投资占比{type:'投资占比',relationship:'投资占比"
                + percent
                + "'}]->(m)"
            )  # 创建关系
            info = "创建关系成功！"
            ...
            graph.run(
                ...
                + '"}),(m:公司名称{name:"'
                + child
                + "\"}) create (n)-[r:投资占比{type:'投资占比',relationship:'投资占比'}]->(m)"
            )
            info = "创建关系成功！"
    else:
        info = "该关系已存在！"
