...
def get_business():
    ...
    nodes = list(map(buildNodes, graph.run("match (a:公司名称{name:\'" + name + "\'})-[b:发行债券|发行股票|总授信额度]-(c) return  "
                                                      " distinct id(c) as id, c.name as name, labels(c) as label")))
    if(len(nodes) == 0):
        nodes = []
        edges = []
    else:
        current_node = list(map(buildNodes, graph.run('match (a:公司名称{name:\"' + name + '\"}) '
        ...
        edges = list(map(buildEdges, graph.run(
            "match (a:公司名称{name:\"" + name + "\"})-[b:发行债券|发行股票|总授信额度]-(c) return distinct id(startnode(b)) as start,id(endnode(b)) as end,type(b) as type, b.relationship as relationship, id(b) as id")))
        #print(nodes, edges)
...
def get_connect():
    ...
    name2 = str(request.args.get('name2'))
    #print(name1, name2)
    nodes = list(map(buildNodes, graph.run("match (a:公司名称{name:\'" + name1 + "\'})-[b:发行债券|发行股票|总授信额度]-(c)-" \
                                                        "[d:上市推荐人|保荐机构|债券主承销商|股票主承销商|合作银行]-(e)-"
                                                "[f:上市推荐人|保荐机构|债券主承销商|股票主承销商|合作银行]-(g)-[h:发行债券|发行股票|总授信额度]-"
                                                "(i:公司名称{name:\'" + name2 + "\'}) return distinct  id(c) as id, c.name as name, labels(c) as label")))
    nodes1 = list(map(buildNodes, graph.run("match (a:公司名称{name:\'" + name1 + "\'})-[b:发行债券|发行股票|总授信额度]-(c)-" \
                                                        "[d:上市推荐人|保荐机构|债券主承销商|股票主承销商|合作银行]-(e)-"
                                                "[f:上市推荐人|保荐机构|债券主承销商|股票主承销商|合作银行]-(g)-[h:发行债券|发行股票|总授信额度]-"
                                                        "(i:公司名称{name:\'" + name2 + "\'}) return distinct  id(e) as id, e.name as name, labels(e) as label")))
    nodes2 = list(map(buildNodes, graph.run("match (a:公司名称{name:\'" + name1 + "\'})-[b:发行债券|发行股票|总授信额度]-(c)-" \
                                                        "[d:上市推荐人|保荐机构|债券主承销商|股票主承销商|合作银行]-(e)-"
                                                "[f:上市推荐人|保荐机构|债券主承销商|股票主承销商|合作银行]-(g)-[h:发行债券|发行股票|总授信额度]-"
    ...
    if (len(nodes) == 0):
        ...
        edges = list(map(buildEdges, graph.run("match (a:公司名称{name:\'" + name1 + "\'})-[b:发行债券|发行股票|总授信额度]-(c)-" \
                                                        "[d:上市推荐人|保荐机构|债券主承销商|股票主承销商|合作银行]-(e)-"
                                                "[f:上市推荐人|保荐机构|债券主承销商|股票主承销商|合作银行]-(g)-[h:发行债券|发行股票|总授信额度]-"
                                                 "(i:公司名称{name:\'" + name2 + "\'}) return distinct  id(startnode(b)) as start, id(endnode(b)) as end, type(b) as type, b.relationship as relationship,id(b) as id")))
        edges1 = list(map(buildEdges, graph.run("match (a:公司名称{name:\'" + name1 + "\'})-[b:发行债券|发行股票|总授信额度]-(c)-" \
                                                        "[d:上市推荐人|保荐机构|债券主承销商|股票主承销商|合作银行]-(e)-"
                                                "[f:上市推荐人|保荐机构|债券主承销商|股票主承销商|合作银行]-(g)-[h:发行债券|发行股票|总授信额度]-"
                                                 "(i:公司名称{name:\'" + name2 + "\'}) return distinct  id(startnode(d)) as start, id(endnode(d)) as end, type(d) as type, d.relationship as relationship, id(d) as id")))
        edges2 = list(map(buildEdges, graph.run("match (a:公司名称{name:\'" + name1 + "\'})-[b:发行债券|发行股票|总授信额度]-(c)-" \
                                                        "[d:上市推荐人|保荐机构|债券主承销商|股票主承销商|合作银行]-(e)-"
                                                "[f:上市推荐人|保荐机构|债券主承销商|股票主承销商|合作银行]-(g)-[h:发行债券|发行股票|总授信额度]-"
                                                 "(i:公司名称{name:\'" + name2 + "\'}) return distinct  id(startnode(f)) as start, id(endnode(f)) as end, type(f) as type, f.relationship as relationship, id(f) as id")))
        edges3 = list(map(buildEdges, graph.run("match (a:公司名称{name:\'" + name1 + "\'})-[b:发行债券|发行股票|总授信额度]-(c)-" \
                                                        "[d:上市推荐人|保荐机构|债券主承销商|股票主承销商|合作银行]-(e)-"
                                                "[f:上市推荐人|保荐机构|债券主承销商|股票主承销商|合作银行]-(g)-[h:发行债券|发行股票|总授信额度]-"
                                                 "(i:公司名称{name:\'" + name2 + "\'}) return distinct  id(startnode(h)) as start, id(endnode(h)) as end, type(h) as type, h.relationship as relationship, id(h) as id")))
        edges = edges + edges1 + edges2 + edges3
    print(nodes, edges)
    return jsonify({'nodes': nodes, 'relations': edges})
...
def get_executive():
    name = str(request.args.get('name'))
    nodes = list(map(buildNodes, graph.run(
        "match (a:公司名称{name:\'" + name + "\'})-[b:高管关系|投资占比]-(c) return distinct id(c) as id, c.name as name, labels(c) as label")))
    nodes = nodes
    if (len(nodes) == 0):
        nodes = []
        edges = []
        ...
        nodes.append((list(map(buildNodes, graph.run(
        "match (c:公司名称{name:\'" + name + "\'}) return distinct  id(c) as id, c.name as name, labels(c) as label"))))[0])
        edges = list(map(buildEdges, graph.run(
        "match (a:公司名称{name:\'" + name + "\'})-[b:高管关系|投资占比]-(c)  return distinct  id(startnode(b)) as start, id(endnode(b)) as end,type(b) as type, b.relationship as relationship, id(b) as id")))
...
def get_fin():
    name = str(request.args.get('name'))
    nodes = list(map(buildNodes, graph.run(
        "match (a:公司名称{name:\"" + name + "\"})-[b:发行债券|发行股票|总授信额度]-(c)-"
                                       "[d:合作银行|上市推荐人|股票主承销商|债券主承销商|保荐机构]-(e) return distinct id(c) as id, c.name as name, labels(c) as label")))
    nodes1 = list(map(buildNodes, graph.run(
        "match (a:公司名称{name:\"" + name + "\"})-[b:发行债券|发行股票|总授信额度]-(e)-[d:合作银行|上市推荐人|股票主承销商|债券主承销商|保荐机构]-(c) return distinct id(c) as id, c.name as name, labels(c) as label")))
    nodes = nodes + nodes1
    if (len(nodes) == 0):
        nodes = []
        edges = []
        ...
        edges = list(map(buildEdges, graph.run(
        "match (a:公司名称{name:\"" + name + "\"})-[b:发行债券|发行股票|总授信额度]-(c)-[d:合作银行|上市推荐人|股票主承销商|债券主承销商|保荐机构]-(e) return distinct   id(startnode(b)) as start, id(endnode(b)) as end,type(b) as type, b.relationship as relationship, id(b) as id")))
        edges1 = (list(map(buildEdges, graph.run(
        "match (a:公司名称{name:\"" + name + "\"})-[b:发行债券|发行股票|总授信额度]-(c)-[d:合作银行|上市推荐人|股票主承销商|债券主承销商|保荐机构]-(e) return distinct  id(startnode(d)) as start, id(endnode(d)) as end,type(d) as type, d.relationship as relationship, id(d) as id"))))
        edges = edges + edges1
    print(nodes, edges)
    return jsonify({'nodes': nodes, 'relations': edges})
    # return jsonify(nodes,  edges)
...
def get_credit():
    name = str(request.args.get('name'))
    nodes = list(map(buildNodes, graph.run("match (a:公司名称{name:\"" + name + "\"})-[b]-(c:总授信额度)-[d]-(e) "
                                                                            "return distinct  id(c) as id, c.name as name, labels(c) as label")))
    nodes1 = list(map(buildNodes, graph.run(
        "match (a:公司名称{name:\"" + name + "\"})-[b]-(e:总授信额度)-[d]-(c) return distinct  id(c) as id, c.name as name, labels(c) as label")))
    nodes = nodes + nodes1
    if (len(nodes) == 0):
        nodes = []
        edges = []
        ...
        "match (c:公司名称{name:\"" + name + "\"}) return  distinct id(c) as id, c.name as name, labels(c) as label"))))[0])
        edges = list(map(buildEdges, graph.run(
        "match (a:公司名称{name:\"" + name + "\"})-[b]-(c:总授信额度)-[d]-(e) return distinct  id(startnode(b)) as start, id(endnode(b)) as end,type(b) as type, b.relationship as relationship,id(b) as id")))
        edges1 = (list(map(buildEdges, graph.run(
        "match (a:公司名称{name:\"" + name + "\"})-[b]-(c:总授信额度)-[d]-(e) return distinct  id(startnode(d)) as start, id(endnode(d)) as end,type(d) as type, d.relationship as relationship, id(d) as id"))))
        edges = edges + edges1
    print(nodes, edges)
    return jsonify({'nodes': nodes, 'relations': edges})
...
def get_child():
    name = str(request.args.get('name'))
    nodes = graph.run("match (a:公司名称{name:\""+name+"\"})-[:投资占比]->(c) return distinct c.name as name").data()   #查询数据信息
...
def get_father():
    name = str(request.args.get('name'))
    nodes = graph.run("match (a:公司名称{name:\""+name+"\"})<-[:投资占比]-(c) return distinct  c.name as name").data()
    return jsonify({"data": nodes})


@app.route('/post_insert', methods=['POST'])
def post_insert():
    parent = str(request.form.get('parent'))
    child = str(request.form.get('child'))
    percent = str(request.form.get('percent'))
    print(percent)
    # 先判断节点是否存在
    node_parent = graph.run("match (n:公司名称{name:\"" + parent + "\"}) return n").data()
    if(len(node_parent) == 0): # 节点不存在，插入数据库
        graph.run("create (n:公司名称{name:\"" + parent +"\",label:'公司名称'}) ")
    node_child = graph.run("match (n:公司名称{name:\"" + child + "\"}) return n").data()
    if (len(node_child) == 0):  # 节点不存在，插入数据库
        graph.run("create (n:公司名称{name:\"" + child + "\",label:'公司名称'}) ")
    #先查询是否有关系存在
    rel = graph.run("match(n:公司名称{name:\"" + parent + "\"})-[r:投资占比]-(m:公司名称{name:\"" + child+ "\"}) return r").data()
    if(len(rel) == 0): # 关系不存在
        if(percent): # 如果有投资占比
            graph.run("match (n:公司名称{name:\""+parent+"\"}),(m:公司名称{name:\""+child+"\"}) create (n)-[r:投资占比{type:'投资占比',relationship:'投资占比"+percent+"'}]->(m)")# 创建关系
            info = "创建关系成功！"
        else:
            graph.run("match (n:公司名称{name:\"" + parent + "\"}),(m:公司名称{name:\"" + child + "\"}) create (n)-[r:投资占比{type:'投资占比',relationship:'投资占比'}]->(m)")
            info = "创建关系成功！"
    else:
        info = "该关系已存在！"
    print(parent, child)
